...
def _stabilize_underreaction(value: float) -> float:
    ...
    return clamp01(0.24 + 0.50 * value)
...
def build_opportunity_components(raw: dict[str, float | None], normalized: dict[str, float]) -> dict[str, float]:
    quality_momentum = clamp01(
        0.60 * normalized.get("quality_change", 0.0)
        + 0.40 * normalized.get("quality_acceleration", 0.0)
    )
    reserve_momentum = clamp01(
        0.65 * normalized.get("reserve_change", 0.0)
        + 0.35 * normalized.get("reserve_growth_without_price", 0.0)
    )
    price_lag = clamp01(
        0.72 * normalized.get("price_response_lag_to_quality_shift", 0.0)
        + 0.28 * normalized.get("expected_price_response_gap", 0.0)
    )
    ...
    uncrowded_participation = clamp01(
        0.48 * normalized.get("participation_without_crowding", 0.0)
        + 0.20 * normalized.get("participation_breadth", 0.0)
        + 0.12 * (1.0 - normalized.get("crowding_proxy", 0.0))
