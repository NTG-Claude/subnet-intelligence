...
def build_fragility_components(raw: dict[str, float | None], normalized: dict[str, float], base_components: dict[str, float]) -> dict[str, float]:
    crowding_level = clamp01(
        0.55 * normalized.get("crowding_proxy", 0.0)
        + 0.25 * normalized.get("overreaction_score", 0.0)
    ...
    weak_market_structure = clamp01(
        0.50 * (1.0 - normalized.get("market_structure_floor", 0.0))
        + 0.30 * (1.0 - normalized.get("market_relevance_proxy", 0.0))
        + 0.20 * (1.0 - normalized.get("update_freshness", 0.0))
    )
    fragility = clamp01(
        0.24 * crowding_level
        + 0.24 * concentration_risk
        + 0.22 * thin_liquidity_risk
        + 0.18 * reversal_risk
